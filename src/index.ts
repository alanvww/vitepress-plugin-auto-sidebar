	...
	return dir
		.filter((e) => {
			...
			return (
				...
				fs.statSync(path.resolve(currentRoot ?? '/', itemPath)).isDirectory()
			);
		})
		.map((entry: string | ContentDir) => {
			...
			if (fs.statSync(childDir).isDirectory()) {
				...
				if (options.useFrontmatter) {
					const fm = frontMatter<{ title: string }>(
						fs.readFileSync(path.resolve(currentRoot ?? '/', e), {
							encoding: 'utf-8',
						})
					);
					if (fm) {
...
export function getSidebar(options: Options = {}) {
	...
	options.contentDirs = options?.contentDirs?.length
		? options.contentDirs
		: ['/'];
